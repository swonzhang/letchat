...
class Login
{
	...
	public function login($data, $response){
		...
		$mg = Capsule::table('user')->where('account',$data['account'])->first();
		...
		if($mg->password != $data['password']){
			throw new \Exception("wrong password", -1);
			
		}

		$json = json_encode(['code'=>0,'content' => ['name'=>$mg->name],'msg' => '']);
		$response->end($json);
	}
